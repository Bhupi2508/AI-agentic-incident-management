...
def validate_closure(feedback: str, system_logs: str) -> str:
    ...
    positive_feedback_patterns = [
        r"issue\s+(is\s+)?(fixed|resolved|closed|done)",
        r"working\s+(fine|properly|normally|well|great)",
        r"service\s+(is\s+)?(fine|okay|ok|up|running)",
        r"(problem|error|bug)\s+(is\s+)?(gone|resolved|not\s+seen|fixed)",
        r"no\s+(issues|errors|problems|bugs)\s+now",
        r"(functionality|app|system)\s+is\s+(restored|normal|working)"
    ]

    positive_logs_patterns = [
        r"no\s+errors",
        r"logs?\s+(are\s+)?clean",
        r"running\s+(smoothly|fine|normally|well|great)",
    ...
    negative_feedback_patterns = [
        ...
        r"not\s+(fixed|resolved|working|done)",
    ...
    print(f"Checking feedback: {feedback}")
    print(f"Positive feedback matched: {matches_any(positive_feedback_patterns, feedback)}")
    print(f"Negative feedback matched: {matches_any(negative_feedback_patterns, feedback)}")
    print(f"Checking system logs: {system_logs}")
    print(f"Positive logs matched: {matches_any(positive_logs_patterns, system_logs)}")
    print(f"Negative logs matched: {matches_any(negative_logs_patterns, system_logs)}")

    # Check negative indicators first (priority)
